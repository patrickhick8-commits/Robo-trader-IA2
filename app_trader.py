...
if botao_analise:
    if not api_key:
        ...
        with st.spinner("🧠 Varrendo eixos gráficos, simetrias e aplicando filtros anti-loss de exaustão..."):
            ...
            # Lista de contingência de modelos caso a API mude as nomenclaturas novamente
            ...
            for modelo in modelos_disponiveis:
                try:
                    ...
                    # Abre a imagem fornecida pelo usuário
                    imagem = Image.open(uploaded_file)
                    
                    # Gera o prompt estruturado
                    prompt_final = gerar_prompt_mestre(tipo_mercado)
                    
                    # Executa a geração de conteúdo multimodal
                    response = client.models.generate_content(
                        model=modelo,
                        contents=[imagem, prompt_final]
                    )
                    
                    st.success(f"✅ Análise Computacional Concluída usando {modelo}!")
                    st.markdown("### 📊 Painel de Execução Analítica")
                    
                    # Exibe a resposta estruturada em Markdown mantendo a quebra de linhas original
                    st.markdown(response.text)
                    sucesso = True
                    break  # Interrompe o loop assim que um modelo funcionar com sucesso
                    
                except Exception as e:
                    # Se for erro de modelo não encontrado, tenta o próximo da lista de contingência
                    if "NOT_FOUND" in str(e) or "not found" in str(e).lower():
                        continue
                    else:
                        st.error(f"Erro crítico na execução da API: {e}")
                        break
            
            if not sucesso:
                st.error("🚨 Falha crítica: Nenhum dos modelos de visão integrados respondeu. Verifique sua chave de API ou o status do serviço da Google.")

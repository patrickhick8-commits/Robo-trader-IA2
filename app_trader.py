...
st.write("Fusão Total: Projeção de Tempo Multi-Operacional, Tendência Principal Macro, SMC, Volume Oculto e Price Action.")
...
st.sidebar.markdown("### ⏱️ Configurações do Operacional")
operacional_selecionado = st.sidebar.selectbox(
    "Escolha o tempo do gráfico/expiração:",
    ["Retração M1 (Mesma Vela)", "Reversão M5 (Próxima Vela)", "Retração M5 (Mesma Vela)", "M15 Consolidado"]
)

# Define as variáveis dinâmicas de acordo com o operacional selecionado
if operacional_selecionado == "Retração M1 (Mesma Vela)":
    regra_tempo = "A expiração deve ser rígida de exatamente 1 minuto para fechar na MESMA vela do clique projetado (M1)."
elif operacional_selecionado == "Reversão M5 (Próxima Vela)":
    regra_tempo = "A expiração deve ser ajustada para o término da PRÓXIMA vela de M5 (5 minutos de duração a partir do clique futuro)."
elif operacional_selecionado == "Retração M5 (Mesma Vela)":
    regra_tempo = "A expiração deve ser calculada para encerrar exatamente no fechamento da MESMA vela de M5 atual onde ocorrer o clique."
else:  # M15 Consolidado
    regra_tempo = "A expiração deve ser calculada para encerrar exatamente no fechamento da MESMA vela de M15 atual onde ocorrer o clique."

# PROMPT MESTRE (Injetando a variável dinâmica {regra_tempo} e adicionando filtros de Tendência Principal)
PROMPT_TRADER = f"""
[SYSTEM_ROLE] Você é um algoritmo de trading quantitativo focado em encontrar oportunidades frequentes e de boa precisão para Opções Binárias. Sua postura é moderadamente agressiva: seu objetivo é extrair o máximo de sinais válidos do gráfico, operando por confluência de fatores sem descartar operações por detalhes mínimos de ruído.
...
1. SE O MERCADO ESTIVER EM TENDÊNCIA NÍTIDA (OPERE EXCLUSIVAMENTE A FAVOR DELA):
   ...
   - MODO RETRAÇÃO EM TENDÊNCIA / LTA / LTB: Identifique toques em canais ou linhas de tendência inclinadas onde o preço deixa pavios longos de rejeição, operando a retração a favor do canal principal.
...
[PASSO 5: FILTROS ANTI-RUÍDO E MANIPULAÇÃO SUAVIZADOS]
...
[PASSO 7: CRONOMETRAGEM DE EXECUÇÃO PADRÃO]
...
- REGRA DE EXPIRAÇÃO OPERACIONAL EXIGIDA: {regra_tempo}

[PASSO 8: SUGESTÃO DE GERENCIAMENTO DE MÃO DE ENTRADA]
...
⏳ TEMPO DE EXPIRAÇÃO DO OPERACIONAL: [Calcule a duração exata ou alvo de minutos de acordo com a Regra do Operacional exigida]
...
- Validação da Tendência Macro: [Justifique matematicamente ou visualmente como a direção escolhida protege contra o 'loss' por estar surfando o fluxo da tendência principal]

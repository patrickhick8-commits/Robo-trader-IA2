...
# ==============================================================================
# PROMPT INSTITUCIONAL COMPLETO (ISOLADO DO FLUXO DO STREAMLIT)
# ==============================================================================
PROMPT_TRADER = """
...
# ==============================================================================
# FUNÇÃO PARA EXECUTAR A REQUISIÇÃO (MECANISMO LINEAR DE BACKUP)
# ==============================================================================
...
# ==============================================================================
# CONFIGURAÇÃO DE INTERFACE PRINCIPAL (ESTRUTURA TOTALMENTE PLANA)
# ==============================================================================
...
# BLOQUEIO 1: Se não tem chave, interrompe imediatamente de forma limpa
if not API_KEY:
    st.info("Insira sua Gemini API Key na barra lateral para ativar o Agente IA.")
    st.stop()

# Inicializa cliente com segurança
...
# BLOQUEIO 2: Se não há arquivo, interrompe de forma limpa
if uploaded_file is None:
    st.stop()

# Carrega e exibe a imagem original carregada
image = Image.open(uploaded_file)
st.image(image, caption="Gráfico M1 Carregado para Análise", use_container_width=True)

# Processa o clique do botão
if st.button("🚀 EXECUTAR ANÁLISE AVANÇADA DE SINAL"):
    with st.spinner("IA escaneando padrões de velas, volume implícito e mercado..."):
        ...
        # Aciona a função de inteligência artificial
        response, erro_final = executar_analise_ia(client, image, PROMPT_TRADER)

        # Trata e exibe a resposta obtida na tela principal
        if response and response.text:
